...
function serveConfigurePage(res, user, token) {
  ...
  const safeState = escapeHtmlClient(
    user.enabled !== false && !left.expired ? 'ACTIVO' : 'CADUCADO / INACTIVO'
  );
  const safeExpires = escapeHtmlClient(
    user.expiresAt ? new Date(user.expiresAt).toLocaleDateString() : 'Sin fecha'
  );
...
async function refreshServer() {
  ...
  try {
    ...
    cacheVersion = (data.version || Date.now()) + '-' + Date.now();
    await drawQr();
...
function reinstallAddon() {
  const cleanUrl = location.origin.replace(/^https?:\\/\\//, '');
  const webUrl = location.origin + manifestUrl();
  const deepLink = 'stremio://' + cleanUrl + manifestUrl();
